package com.leek.atong.simplex.controller;

import com.leek.atong.simplex.util.MatrixOperation;
...
/**
 * @description  单纯形法求解线性规划
 * @author atong
 * @date 15:49 2020/12/7
 * @version 1.0.0.1
 **/
@Controller
public class SimplexMethodController {

    private static final Logger logger = LoggerFactory.getLogger(SimplexMethodController.class);

    @GetMapping("simplex")
    public String simplex() {
        logger.info("localhost:8091/simplex is running...");
        return "index";
    }
    ...
    public String upload(@RequestParam("file") MultipartFile file, Model model) {
        //用于文件输入的辨别变量，1代表C，2代表A，3代表b
        int jud = 1;
        //为0时唯一最优解，为1时无穷多解，为2时无界解,为3时无解
        int type_Solution = 0;
        //用于读取文件时存储c
        ArrayList<Double> c_ = new ArrayList<>();
        //用于读取文件时存储A
        ArrayList<ArrayList<Double>> A_ = new ArrayList<>();
        //用于读取文件时存储b
        ArrayList<Double> b_ = new ArrayList<>();
        //用于读取文件时存储约束的符号
...
//                                msg.append("无解" + "\n");
                ...
                //输出每次循环的结果
                u = u + 1;
//                msg.append("第" + u + "次迭代结果：" + "\n");
                msgs.add("第" + u + "次迭代结果：");
//                msg.append("[基变量]" + "\n");
                msgs.add("[基变量]");
                for(int i = 0; i < x_B.length; i++){
//                    msg.append(x_B_Num[i] + " \n");
//                    msg.append(x_B[i] + "\n");
                    msgs.add(x_B_Num[i] + "  " + x_B[i]);
                }
//                msg.append("[目标函数] " + "\n");
                try{
//                    msg.append(MatrixOperation.oneD_Matrix_Multiplication(c_B,x_B) + "\n");
                    msgs.add("[目标函数] " + MatrixOperation.oneD_Matrix_Multiplication(c_B,x_B) + "");
                }catch (Exception e){
                    e.printStackTrace();
                }
            ...
            if(type_Solution == 0){ //若为唯一最优解，则输出最优解和目标函数
                //输出最终结果
                u = u + 1;
//                msg.append("第" + u +"次迭代结果(最终结果)：" + "\n");
                msgs.add("第" + u +"次迭代结果(最终结果)：");
//                msg.append("\n");
//                msg.append("[基变量]" + "\n");
                msgs.add("[基变量]");
                for(int i = 0; i < x_B.length; i++){
...
//                msg.append("[目标函数]" + "\n");
                ...
                try{
//                    msg.append(MatrixOperation.oneD_Matrix_Multiplication(c_B,x_B) + "\n");
                    msgs.add(MatrixOperation.oneD_Matrix_Multiplication(c_B,x_B) + "");
                }catch (Exception e){
                    e.printStackTrace();
                }
            }
